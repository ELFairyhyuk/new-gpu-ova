 ...
 */
vector<int> MultiPredictor::predict(const vector<vector<KeyValue> > &v_vSamples, real &minPosDecVal, real &minNegDecVal, const vector<int> &vnOriginalLabel) const{
	int nrClass = model.nrClass;
    int manyClassIns=0;
            ...
            for (int i = 0; i < nrClass; ++i) {
                for (int j = i + 1; j < nrClass; ++j) {
                    if (decisionValues[l][k] > 0){
                    	votes[i]++;
		        minPosDecVal=minPosDecVal<decisionValues[l][k]?minPosDecVal:decisionValues[l][k];
		    }
                    else{
                    	votes[j]++;
 			minNegDecVal=minNegDecVal<decisionValues[l][k]?minNegDecVal:decisionValues[l][k];
                }
		k++;
		}
            }
